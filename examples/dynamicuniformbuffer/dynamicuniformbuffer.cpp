...
public:
	...
	uint32_t indexCount;
    uint32_t object_instances_count  =125;
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			for (uint32_t j = 0; j < object_instances_count; j++)
			{
	...
	void prepareUniformBuffers()
	{
		...
		size_t bufferSize = object_instances_count * dynamicAlignment;
...
//			rotationSpeeds->push_back(glm::vec3(rndDist(rndEngine), rndDist(rndEngine), rndDist(rndEngine)));
	...
	void updateDynamicUniformBuffer(bool force = false)
	{
		...
		uint32_t dim = static_cast<uint32_t>(pow(object_instances_count, (1.0f / 3.0f)));
...
Java_de_saschawillems_vulkanSample_JniInterface_ChangeDisplayCount(JNIEnv *env, jclass classz,
                                                                   jint count) {
    // TODO: implement ChangeDisplayCount

	LOGCATI("zwtest ChangeDisplayCount  count %d",count);
    vulkanExample->ChangeObjectInstanceCount(count);
